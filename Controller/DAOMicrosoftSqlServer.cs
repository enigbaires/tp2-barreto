    ...
    public class DAOMicrosoftSqlServer
    {
        public String BuscarMarca(int nroMarca)
        {
            ...
            try
            {
                ...
                command.CommandText = "select Descripcion from MARCAS where Id = " + nroMarca;
                command.Connection = connection;
                connection.Open();
                dataReader = command.ExecuteReader();
                while(dataReader.Read())
                {
                    marca.descripcion = dataReader.GetString(0);
                    result = marca.descripcion;
                    connection.Close();
                    return result;
                }
            }
            ...
            {
                MessageBox.Show("Falló BuscarMarca: " + ex.ToString());
            }
            return result;
        }
        ...
        public String BuscarCategoria(int nroCategoria)
        {
            ...
            SqlCommand command = new SqlCommand();
            SqlDataReader dataReader;
            String result = "No se encontró categoría";
            try
            {
                connection.ConnectionString = "data source = YY118297\\SQLEXPRESS; initial catalog = CATALOGO_DB;integrated security = sspi;";
                command.CommandType = System.Data.CommandType.Text;
                command.CommandText = "select Descripcion from CATEGORIAS where Id = " + nroCategoria;
                command.Connection = connection;
                connection.Open();
                dataReader = command.ExecuteReader();
                ...
                while(dataReader.Read())
                {
                    categoria.descripcion = dataReader.GetString(0);
                    result = categoria.descripcion;
                    connection.Close();
                    return result;
                }
            }
            ...
            {
                MessageBox.Show("Falló BuscarCategoria: " + ex.ToString());
            }
            return result;
        }
